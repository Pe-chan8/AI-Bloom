...
module Ai
  class GeneratePraiseSummaryJob < ApplicationJob
    queue_as :default
    ...
    def perform(post_id:, user_id:, buddy_id:, placeholder_id:)
      ...
      Ai::PraiseSummaryService.new.generate_for(post: post, user: user, buddy: buddy)
      ...
      if ai_message.blank?
        ai_message = AiMessage.create!(
          ...
          post: post,
          kind: :reply,
          content: "ごめんね、まとめを作れなかった…🙏 もう一度押してみてね。"
        )
      end
